...
public class GeneratorService {
    ...
    public List<CodeFile> generate(GeneratorParam generatorParam, DataSourceConfig dataSourceConfig) {
        ...
        for (SQLContext sqlContext : contextList) {
            ...
            for (int tcId : generatorParam.getTcIds()) {
                ...
                String content = doGenerator(sqlContext, template.getContent(), generatorParam.getModelName());

                CodeFile codeFile = new CodeFile(tableName, template.getName(), content);
                /*
                 * 代码生成文件 文件存放路径为：包名+文件名称+文件名称+文件后缀类型 1.包名不为空，则利用用户自定义包名 2.包名为空，则生成AutoCodeSource目录
                 */
                String CodeDirPath = "";
                if (generatorParam.getPackageName().length() > 0) {
                    CodeDirPath = (generatorParam.getPackageName() + "/"
                                   + generatorParam.getPackageName().replaceAll("\\.", Matcher.quoteReplacement("/"))
                ...
                try {
                    // System.out.println(generatorParam.getModelName());
                    String fPath = "";
                    if (generatorParam.getModelName() != "") {
                        createDir(directory.getCanonicalPath() + "/" + CodeDirPath + "/"
                                  + generatorParam.getModelName());
                        fPath = directory.getCanonicalPath() + "/" + CodeDirPath + "/" + generatorParam.getModelName();
                    } else {
                        createDir(directory.getCanonicalPath() + "/" + CodeDirPath);
                        fPath = directory.getCanonicalPath() + "/" + CodeDirPath;
                    }

                    String tpModel = "";
                    if (template.getModel() != null) {
                        tpModel = template.getModel();
                    } else {
                        tpModel = "";
                    }


                    File f = new File(fPath + "/" + sqlContext.getJavaBeanName() + tpModel + "." + template.getSuffix());
                    writeFile(f, content);
                } catch (IOException e) {
                    e.printStackTrace();
                }
    ...
    private String doGenerator(SQLContext sqlContext, String template,String ModelName) {
